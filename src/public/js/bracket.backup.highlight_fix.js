(()=> {
  ...
  // Dibuja una "llave": horizontales desde A y B -> xMid, columna vertical en xMid, y flecha xMid -> T
  function drawBracketConnector(svg, baseEl, A, B, T, emph=false){
    ...
    const xMid = Math.max(xA, xB) + 22;    // separación horizontal de la columna
    const yTop = Math.min(yA, yB);
    const yBot = Math.max(yA, yB);

    // horizontales desde A y B hasta xMid
    line(svg, `M${xA},${yA} L${xMid},${yA}`, emph, false);
    line(svg, `M${xB},${yB} L${xMid},${yB}`, emph, false);

    // columna vertical que une ambas
    line(svg, `M${xMid},${yTop} L${xMid},${yBot}`, emph, false);

    // horizontal desde columna hasta entrada del match destino (con flecha)
