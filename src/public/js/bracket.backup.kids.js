(()=> {
  ...
  /* ---------- API -> UI con metadatos de narrativa ---------- */
  function mapApiToUi(data){
    ...
    const winners=(data.winners||[]).map((round,ri)=>({
      name: ri===0?"Ronda 1 · Cuartos": ri===1?"Ronda 2 · Semifinales": `Ronda ${ri+1}`,
      matches: round.map((m,mi)=>{
        ...
        const nextId = (ri < (data.winners.length-1)) ? `R${ri+2}M${Math.floor(mi/2)+1}` : null;
        return {
          id:m.id, ri, mi, nextId,
          a:m.a??"—", b:m.b??"—",
  ...
  function chipStatus(st){
    ...
    const span = el("span",`chip ${cls}`); span.textContent = map[st] || st;
    return span;
  }
  ...
  function matchNode(m, me){
    ...
    // encabezado: Match ID + chip estado
    const head = el("div","match-head");
    const left = el("div"); left.textContent = `Match ${m.id}`;
    ...
    if(m.nextId){
      ...
      flow.innerHTML = `Ganador → <b>${m.nextId}</b>`;
  ...
  /* ---------- Conectores curvos; énfasis si participa mi equipo ---------- */
  function drawConnectors(container, svg, columns){
    ...
    for(let r=0;r<columns.length-1;r++){
      ...
      for(let i=0;i<to.length;i++){
        ...
        // si el match origen o destino es “you”, resalta el camino
        if(A.classList.contains("you") || B.classList.contains("you") || T.classList.contains("you")){
