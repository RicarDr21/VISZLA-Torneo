(()=> {
  ...
  const roundName = (idx,total)=> {
    if (total===3) return ["Ronda 1 · Cuartos","Ronda 2 · Semifinales","Ronda 3 · Final"][idx] || `Ronda ${idx+1}`;
    if (total===2) return ["Ronda 1 · Semifinales","Ronda 2 · Final"][idx] || `Ronda ${idx+1}`;
    return `Ronda ${idx+1}`;
  };

  function mapApiToUi(data){
    const status=(a,b)=>(Number.isFinite(a)&&Number.isFinite(b))?"done":"pending";
    const total=(data.winners||[]).length;
    const winners=(data.winners||[]).map((round,ri)=>({
      name: roundName(ri,total),
      matches: round.map((m,mi)=>{
        ...
        if(st==="done"){
          ...
          else if(m.scoreA>m.scoreB) winSide="A"; else if(m.scoreB>m.scoreA) winSide="B";
        }
        const nextId = (ri < total-1) ? `R${ri+2}M${Math.floor(mi/2)+1}` : null;
        const nextLabel = (ri < total-1) ? roundName(ri+1,total) : null;
        return { id:m.id, ri, mi, nextId, nextLabel,
          a:m.a??"—", b:m.b??"—", scoreA:m.scoreA??null, scoreB:m.scoreB??null,
          status:st, winSide };
      })
    }));
  ...
  function chipStatus(st){
    const map={pending:"Pendiente", live:"En juego", done:"Finalizado"};
    const cls= st==="done"?"done": st==="live"?"live":"pending";
    const span = el("span",`chip ${cls}`); span.textContent = map[st] || st; return span;
  }
  ...
  function matchNode(m, me){
    ...
    const left = el("div"); left.textContent = `Partido ${m.id}`;
    const right = el("div","chips"); right.appendChild(chipStatus(m.status));
    ...
    if(m.nextId && m.nextLabel){
      const flow = el("div","flow");
      flow.innerHTML = `Ganador → <b>${m.nextLabel}</b> (${m.nextId})`;
  ...
  function renderCol(parent, round, me){
    ...
    col.appendChild(t);
    const arr=[]; round.matches.forEach(m=>{ const n=matchNode(m,me); col.appendChild(n); arr.push(n); });
    parent.appendChild(col);
    return arr;
  }

  function drawConnectors(container, svg, columns, highlight){
    svg.innerHTML=""; if(columns.length<2) return;
    const base = container.getBoundingClientRect();
    const cubic = (x1,y1,x2,y2) => { const mid=(x1+x2)/2; return `M ${x1} ${y1} C ${mid} ${y1}, ${mid} ${y2}, ${x2} ${y2}`; };
    for(let r=0;r<columns.length-1;r++){
      const from=columns[r], to=columns[r+1];
      for(let i=0;i<to.length;i++){
        const A=from[i*2], B=from[i*2+1], T=to[i]; if(!A||!B||!T) continue;
        ...
        if (highlight && (A.classList.contains("you") || B.classList.contains("you") || T.classList.contains("you"))) {
  ...
  function findNext(br, me){
    if(!me) return null;
    for(const r of br.winners){ for(const m of r.matches){ if(m.status==="pending" && (m.a===me||m.b===me)) return m; } }
  ...
  function paint(br){
    ...
    W.innerHTML='<svg class="svg-layer" id="svg-winners"></svg>'; GF.innerHTML="";
    const cols=[]; br.winners.forEach(r=> cols.push(renderCol(W,r,me)));
    GF.appendChild(matchNode(br.grandFinal, me));
    const nm = findNext(br, me);
    NEXT.textContent = nm ? `• ${nm.a} vs ${nm.b} (${nm.id})` : "—";
    requestAnimationFrame(()=> drawConnectors(W, $("#svg-winners"), cols, highlight));
  }
  ...
  (function init(){
    ...
    if($("#hlToggle")) $("#hlToggle").checked = true;
    load($("#tid").value);
  })();
