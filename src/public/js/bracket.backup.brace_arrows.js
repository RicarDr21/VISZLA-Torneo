(()=> {
  ...
  // POSICIONES RELATIVAS AL CONTENEDOR (no al SVG)
  function drawConnector(svg, baseEl, fromEl, toEl, emph=false){
    ...
    const y1 = (f.top + f.bottom)/2 - base.top;
    const x2 = t.left - base.left;
    const y2 = (t.top + t.bottom)/2 - base.top;
  ...
  function render(UI){
    ...
    // svg conectores por encima
    const svg = elNS("svg");
    svg.setAttribute("class","svg");
    svg.setAttribute("id","svg-winners");
    W.appendChild(svg);

    // dibujar después de layout
    requestAnimationFrame(()=>{
      for(let r=0;r<cols.length-1;r++){
        ...
        for(let i=0;i<to.length;i++){
          ...
          drawConnector(svg, W, A, T, emph);
          drawConnector(svg, W, B, T, emph);
        }
      }
    });
  }
